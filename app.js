...
const AppHeader = {
    ...
    template: `
        <!-- Header is now ONLY visible on the Home tab, and the border line is removed -->
            ...
            <div class="modal-content" @touchstart="$emit('touch-start', $event)" @touchmove="$emit('touch-move', $event)" @touchend="$emit('touch-end', $event)">
                ...
                <div v-else>
                    ...
                    <div class="action-menu">
                        <button class="menu-btn sync-row" :style="syncState.includes('SUCCESS') ? 'color: var(--success);' : ''" @click="$emit('sync')">
                            <div class="btn-content">
                                <div class="icon-wrap"><span class="material-symbols-rounded" style="font-size: 18px;">sync</span></div>
                                <span>{{ syncState }}</span>
                            </div>
                        </button>
                        <button class="menu-btn wipe-row" @click="$emit('wipe')">
                            <div class="btn-content">
                                <div class="icon-wrap"><span class="material-symbols-rounded" style="font-size: 18px;">delete</span></div>
                                <span>Wipe Gerald Memory</span>
                            </div>
                        </button>
                        <button class="menu-btn nuke-row" @click="$emit('nuke-cache')">
                            <div class="btn-content">
                                <div class="icon-wrap"><span class="material-symbols-rounded" style="font-size: 18px;">cached</span></div>
                                <span>Nuke App Cache</span>
                            </div>
                        </button>
                        <button class="menu-btn logout-row" @click="$emit('logout')">
                            <div class="btn-content">
                                <div class="icon-wrap"><span class="material-symbols-rounded" style="font-size: 18px;">logout</span></div>
                                <span>Sign Out</span>
                            </div>
                        </button>
                    </div>
...
const ChatView = {
    ...
    template: `
        <!-- HARD FLEX LAYOUT: Padding dynamically reserves exactly enough space for the bottom nav so nothing slides underneath! -->
        <div class="chat-wrapper" style="flex: 1; display: flex; flex-direction: column; overflow: hidden; padding-bottom: calc(85px + env(safe-area-inset-bottom, 0px));">
            ...
            <!-- Scroll area safely sandwiched between header and input bar -->
            <div class="twitch-chat-list" id="twitch-chat-list" @click="closePicker" style="flex: 1; overflow-y: auto; padding: 10px 12px;">
            ...
            <div class="chat-emote-tray" v-show="showPicker && isLoggedIn" @click.stop style="position: absolute; bottom: calc(60px + 85px + env(safe-area-inset-bottom, 0px)); z-index: 70;">
            ...
            <!-- Overriding absolute CSS so it naturally blocks scroll bleeding -->
            <div class="custom-chat-input-area" style="position: relative; bottom: auto; z-index: 60; flex-shrink: 0; padding-bottom: 10px;">
...
const MoreView = {
    template: `
        <!-- Reserved padding at bottom ensures we do not scroll into navbar -->
        <div class="more-container" style="flex: 1; overflow-y: auto; padding: 12px 16px; padding-bottom: calc(85px + env(safe-area-inset-bottom, 0px));">
            
            <a href="https://throne.com/codemiko" target="_blank" style="background: #0ea5e9; color: #fff; border-radius: 12px; padding: 12px 16px; display: flex; align-items: center; justify-content: space-between; text-decoration: none; font-weight: bold; margin-bottom: 8px; font-size: 14px;">
                <span>Throne</span> 
                <svg viewBox="0 0 24 24" style="width: 18px; height: 18px; fill: white;"><path d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/></svg>
            </a>

            <!-- Reduced gaps, fonts, and paddings so all 12 easily fit without scrolling -->
            <div style="display:flex; flex-direction:column; gap:4px;">
                <a href="https://www.twitch.tv/codemiko" target="_blank" class="social-card" style="padding: 8px 12px; border-radius: 10px;">
                    <svg viewBox="0 0 24 24" class="social-icon" style="width: 18px; height: 18px; color: #9146FF;"><path fill="currentColor" d="M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714Z"/></svg>
                    <span style="color: var(--text-main); font-size: 13px;">Twitch</span>
                </a>
                
                <a href="https://www.youtube.com/@CodeMiko" target="_blank" class="social-card" style="padding: 8px 12px; border-radius: 10px;">
                    <svg viewBox="0 0 24 24" class="social-icon" style="width: 18px; height: 18px; color: #FF0000;"><path fill="currentColor" d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                    <span style="color: var(--text-main); font-size: 13px;">YouTube</span>
                </a>
                
                <a href="https://kick.com/codemiko" target="_blank" class="social-card" style="padding: 8px 12px; border-radius: 10px;">
                    <svg viewBox="0 0 24 24" class="social-icon" style="width: 18px; height: 18px; color: #53FC18;"><path fill="currentColor" d="M19 2H5a3 3 0 0 0-3 3v14a3 3 0 0 0 3 3h14a3 3 0 0 0 3-3V5a3 3 0 0 0-3-3zM10.1 14.5v3.3H7.4V6.2h2.7v4.6l3.3-4.6h3.4l-3.9 5.1 4.2 6.5h-3.5z"/></svg>
                    <span style="color: var(--text-main); font-size: 13px;">Kick</span>
                </a>
                
                <a href="https://discord.com/invite/codemiko" target="_blank" class="social-card" style="padding: 8px 12px; border-radius: 10px;">
                    <svg viewBox="0 0 24 24" class="social-icon" style="width: 18px; height: 18px; color: #5865F2;"><path fill="currentColor" d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189Z"/></svg>
                    <span style="color: var(--text-main); font-size: 13px;">Discord</span>
                </a>

                <a href="https://app.fanfix.io/@codeyuna" target="_blank" class="social-card" style="padding: 8px 12px; border-radius: 10px;">
                    <svg viewBox="0 0 24 24" class="social-icon" style="width: 18px; height: 18px;">
                        <defs>
...
const GeraldMinigames = {
    ...
    template: `
        <div class="chat-emote-tray" v-show="showMinigames" style="position: absolute; bottom: 100%; border-bottom:none; border-radius:16px 16px 0 0; background: var(--bg-color);">
            <div style="display:flex; flex-wrap:wrap; gap:8px; padding: 12px; justify-content:center;">
                <!-- Fixed width limits to prevent minigame buttons from exploding in size -->
                <button v-for="g in gameDeck" :key="g.id" class="bribe-btn" style="padding: 8px 12px; font-size: 13px; border-radius: 20px; white-space:nowrap; flex: 0 0 auto; max-width: 180px; background: var(--card-bg); border: 1px solid var(--border-color); color: var(--text-main); font-weight: bold;" @click.stop="$emit('play-game', g)">
                    {{ g.label }}
                </button>
            </div>
        </div>
...
const GeraldView = {
    ...
    template: `
        <div class="gerald-container" style="flex: 1; display: flex; flex-direction: column; overflow: hidden; padding-bottom: calc(85px + env(safe-area-inset-bottom, 0px));">
            ...
            <div class="gerald-messages" id="gerald-msgs" @click="$emit('close-pickers')" style="flex: 1; overflow-y: auto; padding: 10px 16px;">
            ...
            <!-- Overriding Absolute Position to naturally sit above navbar without bleeding -->
            <div class="gerald-action-area" style="position: relative; bottom: auto; flex-shrink: 0; z-index: 50;">
...
const HomeView = {
    ...
    template: `
        <div style="flex: 1; overflow-y: auto; padding-bottom: calc(85px + env(safe-area-inset-bottom, 0px));">
...
createApp({
    ...
    setup() {
        ...
        const updateThemeClass = () => {
            ...
            // Explicitly sync the html/body backgrounds to fix Android scroll-flicker
        ...
        const parseIrcMessage = (raw) => {
            ...
            // Fire-and-forget, quietly fail if ghost user
        ...
        const sendTwitchChatMessage = (msg) => {
            ...
            // Instantly grab your known badges from your last known local message so they don't disappear!
            let myBadges = [];
            const lastMyMsg = chatMessages.value.slice().reverse().find(m => m.username?.toLowerCase() === twitchUsername.value?.toLowerCase() && m.badges?.length > 0);
            if (lastMyMsg) { myBadges = [...lastMyMsg.badges]; }
            ...
            handleLogin: async () => { 
                try {
                    ...
                    if (data.user) { 
                        ...
                        // Explicitly removed the code that closed the modal so it STAYS OPEN
                        const { data: hist } = await sbClient.from('gerald_history').select('*').eq('user_id', currentUser.value.id).order('created_at', { ascending: true });
                        if (hist && hist.length > 0) { 
                            geraldMessages.value = hist.map(r => ({ role: r.role, content: r.content })); 
                        }
