...
const DiscordModal = {
    props: ['isOpen'],
    template: `<div class="modal-overlay" :class="{ open: isOpen }" @click.self="$emit('close')"><div class="modal-content text-center"><h2 style="font-size:20px; font-weight:800; margin-bottom:10px;">Join Discord</h2><p style="color:var(--text-muted); font-size:14px; margin-bottom:20px;">Open CodeMiko's official Discord server.</p><a href="https://discord.com/invite/codemiko" target="_blank" @click="$emit('close')" style="background:var(--discord); color:white; padding:15px; border-radius:12px; text-decoration:none; font-weight:800; display:block; margin-bottom:10px;">LAUNCH DISCORD</a><button @click="$emit('close')" style="background:transparent; border:none; color:var(--text-muted); font-weight:700; width:100%; padding:10px;">CANCEL</button></div></div>`
};

const ClipModal = {
    props: ['clip', 'hostname'],
    template: `<div class="clip-modal-overlay" :class="{ open: !!clip }" @click.self="$emit('close')"><div class="clip-modal-content" v-if="clip"><iframe :src="'https://clips.twitch.tv/embed?clip=' + clip.id + '&parent=' + hostname + '&autoplay=true&muted=false'" allow="autoplay; fullscreen" allowfullscreen></iframe></div></div>`
}
...
createApp({
    ...
    setup() {
        ...
        const connectTwitchChat = () => {
            ...
            twitchWs.onopen = () => {
                ...
                if (twitchChatToken.value) {
                    ...
                    // Fetch username or use anonymous fallback for token-less (read-only)
        ...
        const clearGeraldHistory = async () => {
            ...
            geraldMessages.value = [{role:'gerald', content: geraldGreetings.join('\n')}];
        ...
        return { 
            hostname, splashVisible, splashOpacity, currentTab, clips, modals, isLive, toast, currentUser, loginEmail, loginPass, apiConfig, geraldInput, geraldMessages, isGeraldTyping, talkToGerald, logoSvg, syncState, wipeState, logoutState, runSync, isHeaderVisible, handleScroll, handleModalTouchStart, handleModalTouchMove, handleModalTouchEnd, currentFilter, activeFilterLabel, isFilterMenuOpen, closeFilterMenu, applyFilter, parseMarkdown, shareClip, recentVods, currentVodIndex, nextVod, prevVod, customEmotes, showEmotePicker, insertEmote, clearGeraldHistory, isPulling, refreshTransform, isRefreshing, handlePullStart, handlePullMove, handlePullEnd, handleGeraldEnter, toggleEmotes, toggleMinigames, closePickers, ytFeed, redditFeed, formatNumber, showMinigames, playMinigame, activeFeedSource,
