    ...
    <div class="feed-layout" :class="{ active: isRefreshing }" v-show="currentTab === 'feed'">
        <div class="segment-header-wrapper">
            <div class="segment-tabs-container">
                <button class="segment-tab-btn" :class="{ active: feedSegment === 'youtube' }" @click="feedSegment = 'youtube'"><span class="material-symbols-rounded">play_circle</span>VODS</button>
                <button class="segment-tab-btn" :class="{ active: feedSegment === 'reddit' }" @click="feedSegment = 'reddit'"><span class="material-symbols-rounded">forum</span>REDDIT</button>
            </div>
        </div>
        
        <div id="yt-wrapper" v-if="feedSegment === 'youtube'" style="height: calc(100% - 64px); padding-top: 10px;">
            <div v-if="ytFeed && ytFeed.length > 0">
                <div class="vod-animated-border"><div class="video-container"><iframe v-if="currentTab === 'feed'" :src="'https://www.youtube.com/embed/' + ytFeed[ytCurrentIndex].id" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen loading="lazy"></iframe></div></div>
            ...
            <div v-else class="feed-loader-msg"><span class="material-symbols-rounded spin-anim">sync</span> Fetching from YouTube...</div>
        </div>
        
        <div id="reddit-wrapper" v-if="feedSegment === 'reddit'" style="height: calc(100% - 64px); padding-top: 10px;">
            <div v-if="redditFeed && redditFeed.length > 0" style="display:flex; flex-direction:column; height:100%;">
                <a :href="'https://reddit.com' + redditFeed[redditCurrentIndex].permalink" target="_blank" class="reddit-compact-card">
                    ...
                    <div v-if="redditFeed[redditCurrentIndex].thumbnail && redditFeed[redditCurrentIndex].thumbnail.startsWith('http')" class="reddit-img-container"><img :src="redditFeed[redditCurrentIndex].thumbnail" onerror="this.style.display='none'" alt="Reddit Media"></div>
            ...
            <div v-else class="feed-loader-msg"><span class="material-symbols-rounded spin-anim">sync</span> Fetching from Reddit...</div>
        </div>
    </div>`
};
...
const HomeView = {
    ...
    <div class="scroll-area content-shimmer" :class="{ active: isRefreshing }" id="feed-scroll" v-show="currentTab === 'home'" @scroll="$emit('scroll', $event)">
        <div class="feed-snap-item" data-id="featured">
            ...
            <div class="header-controls">
                ...
                <div class="premium-badge vod" v-else-if="recentVods && recentVods.length > 0"><div class="dot"></div><span>{{ recentVods[currentVodIndex] ? ('VOD • ' + recentVods[currentVodIndex].date) : 'PAST BROADCAST' }}</span></div>
            </div>
        ...
        <div class="feed-snap-item" v-for="(clip, index) in clips" :key="clip.id" :data-id="clip.id">
            ...
            <div class="clips-header" v-if="index === 0">
...
createApp({
    ...
    setup() {
        ...
        const fetchSocialFeeds = () => {
            ...
            // Using allorigins to completely bypass strict browser CORS policies without failing JSON parsing
            fetch('https://api.allorigins.win/get?url=' + encodeURIComponent('https://www.reddit.com/r/CodeMiko/new.json?limit=10'), { signal: abortController.signal })
                .then(res => res.json())
                .then(data => {
            ...
            fetch('https://api.rss2json.com/v1/api.json?rss_url=' + encodeURIComponent('https://www.youtube.com/feeds/videos.xml?channel_id=UCwX993DUC_wXo9E3dKipGAmYg'), { signal: abortController.signal })
                .then(res => res.json())
                .then(data => {
