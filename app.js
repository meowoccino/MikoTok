...
const SplashScreen = {
    ...
    template: `<div id="splash-screen" v-if="splashVisible" :style="{ opacity: splashOpacity }"><div style="width:100px;height:100px;margin-bottom:20px;" v-html="logoSvg('splash')"></div><div class="header-title" style="font-size: 32px; font-weight: 900;">MikoTok</div><div class="progress-bar"><div class="progress-fill"></div></div></div>`
};
...
createApp({
    ...
    setup() {
        ...
        // Re-added activeClipId ref to process native in-place clip rendering
        const activeClipId = ref(null);

        const chatMessages = ref([]);
        const chatInput = ref('');
        const twitchChatToken = ref(localStorage.getItem('tw_chat_token') || null);
        const twitchAuthUrl = ref('https://id.twitch.tv/oauth2/authorize');
        let twitchWs = null;
        ...
        const fetchSocialFeeds = () => {
            ...
            fetch('https://api.allorigins.win/raw?url=' + encodeURIComponent('https://api.rss2json.com/v1/api.json?rss_url=https://www.youtube.com/feeds/videos.xml?channel_id=UCXz_cgB5Lq2kE9Z-2-zU-hg'))
                .then(res => res.json())
                .then(data => {
