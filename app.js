...
const GeraldMinigames = {
    ...
    data() {
        return {
            gameDeck: [
                { id: 'glitch', label: '🕶️ Glitch Persona', prompt: '[SYSTEM INJECTION: Glitch persona override activated. Act broken, hyper-cynical, and target the stream layout!]' },
                { id: 'shader', label: '🔥 Compile UE5', prompt: '[SYSTEM INJECTION: Compilation error simulation. Complain aggressively about system lag, hardware resources, and VRAM limits.]' },
                { id: 'boba', label: '🥤 Boba Spill', prompt: '[SYSTEM INJECTION: Critical emergency alert! Sticky tapioca fluid has entered your cooling fans. React with mechanical panic!]' },
                { id: 'pineapple', label: '🚪 Pineapple Walk-In', prompt: '[SYSTEM INJECTION: External visual disruption detected. Chris has walked in unannounced. Mock the technician\'s complete loss of streaming privacy.]' },
                { id: 'cat', label: '🐈 Cat on PC', prompt: '[SYSTEM INJECTION: Hardware exhaust block! Blue the savannah cat is sitting on your primary fan array. Drop into defensive alert protocols.]' },
                { id: 'bits', label: '🎟️ 100K Bits', prompt: '[SYSTEM INJECTION: Bit transaction flash! A viewer dropped 100,000 bits. Treat this massive transaction animation as a complete system memory flood.]' },
                { id: 'mute', label: '🔇 Mute Mic', prompt: '[SYSTEM INJECTION: Microphonic capture error. The chat muted her mic asset. Celebrate your absolute quietness sarcastically.]' },
                { id: 'bald', label: '🧑‍🦲 Delete Hair', prompt: '[SYSTEM INJECTION: Direct vertex asset manipulation. Optimize engine loads by deleting the technician\'s hair mesh layers. Laugh at her baldness.]' },
                { id: 'siren', label: '🚨 Firetruck Siren', prompt: '[SYSTEM INJECTION: Decibel threshold exceeded! The technician is screaming like a high-frequency emergency vehicle. Complain about ear structural damage.]' }
            ]
        };
    },
    template: `
            ...
            <div class="gerald-header" @click="$emit('close-pickers')">
                ...
                <div class="gerald-sys-card-compressed">
                    ...
                    <div class="ai-status-node-tiny">
                        <div class="pulse-node" :class="geminiStatus === 'ONLINE' ? 'pulse-green' : 'pulse-red'"></div>
                        <span class="pulse-lbl">{{ geminiStatus }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="gerald-messages" id="gerald-msgs" @click="$emit('close-pickers')">
                <template v-for="(m, i) in geraldMessages" :key="i">
                    <div v-if="i === 0 && m.role === 'gerald'" class="chat-bubble gerald startup-anim">
                        <span v-if="!m.content">> GERALD_CORE initialized.<br>> System parameters clear. Command me.</span>
    ...
    setup() {
        ...
        const processEmotes = (text) => {
            ...
            for (let i = 0; i < words.length; i++) {
                ...
                if (match && match.url) { words[i] = `<img src="${match.url}" class="chat-emote-img" title="${cleanWord}">`; }
            }
            return words.join(' ');
        };
        ...
        const triggerAiMinigame = (gameObj) => {
            ...
            contextHistory.push({ role: 'user', parts: [{ text: gameObj.prompt }] });

            sbClient.functions.invoke('gerald-chat', { body: { history: contextHistory } }).then(({ data, error }) => {
                if (!error && data?.reply) {
        ...
        const parseMarkdown = (t) => {
            if (!t) return ''; let html = t.replace(/</g, '<').replace(/>/g, '>');
            html = html.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\*(.*?)\*/g, '<em>$1</em>');
            const words = html.split(' ');
            for (let i = 0; i < words.length; i++) {
                const clean = words[i].replace(/^:|:$/g, ''); const m = customEmotes.value[clean];
                if (m) words[i] = `<img src="${m.url || `https://cdn.discordapp.com/emojis/${m.id}.png`}" style="height:1.65em; vertical-align:middle; display:inline-block; margin:0 2px;">`;
            }
            return words.join(' ');
        };
